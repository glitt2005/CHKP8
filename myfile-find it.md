...
Es un método que toma como input un iterable de promesas que toma  and devuelve una sol a single [`Promise`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise). This returned promise fulfills when all of the input's promises fulfill (including when an empty iterable is passed), with an array of the fulfillment values. It rejects when any of the input's promises rejects, with this first rejection reason.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzQwMzcwMzg4LC0xNDA2MTE0NDIsODA5Mz
